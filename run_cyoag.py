...
def run_game():
    ...
    cyoag_py_path = os.path.abspath(os.path.join('cyoag_git', 'cyoag.py'))
    
    if sys.platform.startswith('linux'):
        # Linux: Try using gnome-terminal or fallback to xterm if gnome-terminal isn't found
        try:
            subprocess.run(['gnome-terminal', '--geometry=300x50', '--', 'python3', cyoag_py_path])
        except FileNotFoundError:
            print("gnome-terminal not found, trying xterm.")
            subprocess.run(['xterm', '-geometry', '300x50', '-e', f'python3 {cyoag_py_path}'])
        ...
        subprocess.run(['start', 'cmd', '/k', f'python {cyoag_py_path}'], shell=True)
    else:
...
if __name__ == "__main__":
    ...
    install_requirements()  # Install missing packages from requirements.txt and run the game